...
m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
	...
	m_bAviInit = false;
	m_dwFourCC = 1935959654;

	m_bSelectCodecByUser = false;
}
...
HRESULT CAVIGenerator::InitEngine()
{
	...
	if(this->m_bSelectCodecByUser)
	{
		...
		this->m_dwFourCC = opts.fccHandler;
		::AfxGetApp()->WriteProfileInt("SMEyeApp", "fourcc", m_dwFourCC);
	...
	{
		m_dwFourCC = ::AfxGetApp()->GetProfileInt("SMEyeApp", "fourcc", m_dwFourCC);


		AVICOMPRESSOPTIONS copts, *pcopts = &copts;
		copts.fccType = streamtypeVIDEO;
		copts.fccHandler = m_dwFourCC; //ffds
		copts.dwKeyFrameEvery = 0; 
		copts.dwQuality = 0; 
	...
	/*char buf[100];
	::ZeroMemory(buf, 100);
	memcpy(buf, &opts.cbFormat, sizeof(DWORD));
	::ZeroMemory(buf, 100);
	memcpy(buf, &opts.fccHandler, sizeof(DWORD));*/
