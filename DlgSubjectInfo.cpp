...
void CDlgSubjectInfo::OnBnClickedOk()
{
	...
	CString strName, strID, strMemo, strAge, strDoctor, strTester;
	int age, sex;
	...
	GetDlgItem(IDC_EDIT_DOCTOR_NAME)->GetWindowText(strDoctor);
	GetDlgItem(IDC_EDIT_TESTER_NAME)->GetWindowText(strTester);


	::ZeroMemory(m_PI.m_chName, MAX_PI_NAME);
	::ZeroMemory(m_PI.m_chID, MAX_PI_ID);
	::ZeroMemory(m_PI.m_chMemo, MAX_PI_MEMO);
	::ZeroMemory(this->m_PI.m_chDoctor, MAX_PI_DOCTOR);
	::ZeroMemory(m_PI.m_chTester, MAX_PI_TESTER);
	...
	memcpy(m_PI.m_chDoctor, strDoctor.GetBuffer(), min(strDoctor.GetLength(), MAX_PI_DOCTOR-1));
	memcpy(m_PI.m_chTester, strTester.GetBuffer(), min(strTester.GetLength(), MAX_PI_TESTER-1));
...
BOOL CDlgSubjectInfo::OnInitDialog()
{
	...
	GetDlgItem(IDC_EDIT_DOCTOR_NAME)->SetWindowText(m_PI.m_chDoctor);
	GetDlgItem(IDC_EDIT_TESTER_NAME)->SetWindowText(m_PI.m_chTester);
