...
*/
	...
	GetDlgItem(IDC_STATIC_SUBJECT_INFO)->MoveWindow(x + sideMargin+10, y+sideMargin+20, group_width-20, group_height-(4*sideMargin+button_h*2));
...
void CSMEyeAppDlg::initTChart(unsigned int showChart)
{
	...
	for(int i=0; i<CHART_NUM; i++)
	{
		...
		//color line tool
		m_ChartCtrl[i].GetTools().Add(tcColorLine);
		VARIANT varAxis;
...
void CSMEyeAppDlg::OnBnClickedCheckCamStart()
{
	...
	else		//stop
	{
		...
		if(this->m_displaySource == DISPLAY_SOURCE_CAM)
		{
			...
			else if(this->m_CommACC.m_bConnected)
			{
				
				
				//Com port에 남은 데이터를 짜낸다.
				Sleep(100);
				this->m_CommACC.stop();
