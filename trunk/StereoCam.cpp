		...
		if(m_Cameras[0].m_bIsNewOne && m_Cameras[1].m_bIsNewOne)
		{
			...
			if(m_bUnderTest)
			{
				...
				double data[15] = {m_EyeInfos[0].m_fCx, m_EyeInfos[1].m_fCx,
									...
									pGyro[0], -1*pGyro[1], pGyro[2], 0, 0,					//수직에는 -1을 곱해줘야 방향이 맞는듯.
				...
				//timestamp를 저장하기 위한 file을 open
				if(this->m_fileTimestamp.m_hFile == CFile::hFileNull)
				{
					CString strFname, appFolder;
					corGetProgramFolder(&appFolder);
					strFname.Format("%s\\%s", appFolder, TIMESTAMPE_FILE_NAME);
					this->m_fileTimestamp.Open(strFname, CFile::modeCreate | CFile::modeWrite);
				}

				// timestamp 저장
				unsigned int timestamp[2] = {this->m_Cameras[0].m_timeCycleCount, m_Cameras[1].m_timeCycleCount};
				if(this->m_fileTimestamp.m_hFile != CFile::hFileNull)
					m_fileTimestamp.Write(timestamp, sizeof(int)*2);
