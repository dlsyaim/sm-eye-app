...
void ED_rptMaker::addText(CString label, int templateIdx)
{
	...
	{
	case 1:
		text = "report_label:0.15,TOP,10.0 ,BOTTOM,TEXT,,0,휴먼엑스포,100,1,0,0,0,0,0,129;\n";
		textHeight = TEXT1_HEIGHT;
		break;
	case 2:
		text = "report_label:0.3,TOP,10.0 ,BOTTOM,TEXT,,0,휴먼엑스포,80,0,0,0,0,0,0,129;\n";
...
{
	CString str(m_patientInfo.m_chMemo);

	USES_CONVERSION;
	WCHAR* pBufw = A2W(m_patientInfo.m_chMemo);
	CStringW strw(pBufw);
	...
	if(str.GetLength())
	{
		...
		while(strw.GetLength())
		{
			int idx = strw.Find('\n');
			if( (idx != -1) && (idx < lineLen))
			{
				addText(W2A(strw.Left(idx)), 2);
				strw.Delete(0, idx+1);
			}
			else
			{
				int idx = min(strw.GetLength(), lineLen);
				addText(W2A(strw.Left(idx)), 2);
				strw.Delete(0, idx+1);
			}
		}
