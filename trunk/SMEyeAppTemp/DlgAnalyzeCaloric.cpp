...
void insertString(CString* pStr, double value, CString ID);
...
void CDlgAnalyzeCaloric::OnBnClickedButtonCaloricPrint()
{
	...
	temp.Format("%s %d",  m_patientInfo.m_nSex ? "Male":"Femail", m_patientInfo.m_nAge);
	strForm.Replace("SUBJECT_SEX_AGE", temp);
	...
	for(int i=0; i<7; i++)
		insertString(&strForm, value[i], ids[i]);
