	...
	if(testFile.open(this->m_strTestFile, &m_testInfo, false))
	{
		...
		if(m_ulDataCount)
		{
			...
			if((this->m_testInfo.m_stiModeCommand == ED_TestProtocole::getStiModeCommand(STI_CALIBRATION)) && (m_testInfo.m_stiSubmode != ED_TestProtocole::getStiSubmodeNum(STI_CALIBRATION)-1) )
				testFile.readTestData(false, this->m_pdLH, m_pdRH, m_pdLV, m_pdRV, 0, 0, 0, 0, 0, 0, 0, 0, 0, this->m_pdStiH, m_pdStiV);
			// gyro calibration
			else	
				testFile.readTestData(false, this->m_pdLH, m_pdRH, m_pdLV, m_pdRV, 0, 0, 0, 0, 0, m_pdStiV, m_pdStiH, 0, 0, 0, 0);
