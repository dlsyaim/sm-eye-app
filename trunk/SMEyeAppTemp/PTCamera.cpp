...
, m_ulFrameNum(0)
, m_dFramerate(0)
, m_nBufTimeStampCount(0)
, m_uiPrevSeqNum(0)
, m_nFrameMissed(0)
, m_bTorsionReset(true)
{
	::memset(m_ulBufTimeStamp, 0 , sizeof(unsigned long)*TIME_STAMP_BUF_LENGTH);	
...
bool CPTCamera::processImage(bool bMakeResult)
{
	...
	//if(this->extractImageData())
	{
		...
		if(m_nUnprocessedImage != 1)
		{
			m_nFrameMissed += m_nUnprocessedImage-1;
			m_nFrameMissedTotal += m_nUnprocessedImage-1;
			//TRACE("__________________________________%d camera : %d Unprocessed Image, Total : %d \n", 
			//	this->m_nCameraIndex, m_nUnprocessedImage-1, m_nFrameMissed);		
		}
		m_nUnprocessedImage = 0;
...
unsigned int CPTCamera::getMissingFrameNum()
{
	return this->m_nFrameMissedTotal;
}
